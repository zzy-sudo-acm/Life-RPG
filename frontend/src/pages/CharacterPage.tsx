  ...
  return (
    <div className="space-y-7">
      <header className="flex flex-col gap-4 border-b border-line pb-5 sm:flex-row sm:items-end sm:justify-between">
        <div><h1 className="font-display text-3xl font-bold text-ink">角色</h1><p className="mt-2 text-sm text-muted">这里集中展示成长反馈；日常无需手动维护数值。</p></div>
        <Button variant="secondary" icon={<Pencil size={15} />} onClick={() => setCharacterOpen(true)}>编辑角色</Button>
      </header>
        ...
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex flex-wrap items-baseline justify-between gap-2">
              <div><h2 className="font-display text-2xl font-bold text-ink">{data.character.name}</h2><p className="mt-1 text-sm text-muted">{data.character.profession} · {data.character.lifeStage}</p></div>
              <p className="text-sm text-muted">总 EXP <strong className="text-exp">{formatNumber(data.character.totalExp)}</strong></p>
            </div>
            <div className="mt-4"><ProgressBar value={data.character.exp} max={data.character.expToNextLevel} tone="exp" size="lg" label={`Lv.${data.character.level} · EXP ${formatNumber(data.character.exp)} / ${formatNumber(data.character.expToNextLevel)}`} /></div>
          </div>
        </div>
      </Panel>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section aria-labelledby="skills-heading">
          <div className="mb-3"><h2 id="skills-heading" className="flex items-center gap-2 font-display text-xl font-bold text-ink"><BookOpen size={19} className="text-primary" /> 技能</h2><p className="mt-1 text-xs text-faint">完成同分类任务时自动成长</p></div>
        ...
        <section aria-labelledby="stats-heading">
          <div className="mb-3"><h2 id="stats-heading" className="flex items-center gap-2 font-display text-xl font-bold text-ink"><TrendingUp size={19} className="text-primary" /> 五维属性</h2><p className="mt-1 text-xs text-faint">分类决定任务主要成长方向</p></div>
          <Panel className="p-4 sm:p-5">
            <ul className="space-y-4">
              {STAT_KEYS.map((key) => <li key={key} className="flex items-center gap-4"><span className="w-16 shrink-0 text-sm text-muted">{STAT_LABELS[key]}</span><div className="h-2 flex-1 overflow-hidden rounded-full bg-raised"><div className="h-full rounded-full bg-primary" style={{ width: `${Math.min(100, data.stats.values[key])}%` }} /></div><strong className="w-12 text-right tabular-nums text-ink">{data.stats.values[key]}</strong></li>)}
            </ul>
          </Panel>
        </section>
      </div>
      ...
      <section aria-labelledby="achievements-heading">
        <div className="mb-3"><h2 id="achievements-heading" className="flex items-center gap-2 font-display text-xl font-bold text-ink"><Award size={19} className="text-exp" /> 成就</h2><p className="mt-1 text-xs text-faint">重要节点会自动解锁并写入成长足迹</p></div>
        {unlockedAchievements.length > 0 ? (
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {unlockedAchievements.map((achievement) => <Panel key={achievement.id} className="flex items-start gap-3 p-4"><span className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-exp-soft text-2xl">{achievement.icon}</span><div><h3 className="font-semibold text-ink">{achievement.name}</h3><p className="mt-1 text-xs leading-5 text-muted">{achievement.description}</p><p className="mt-2 text-[11px] text-faint">{formatDate(achievement.unlockedAt)}</p></div></Panel>)}
          </div>
        ) : <Panel className="p-6 text-sm text-muted">成就尚未解锁。持续完成任务和长期目标即可自然获得。</Panel>}
      </section>

      <section aria-labelledby="footprint-heading">
        <div className="mb-3 flex items-end justify-between gap-3"><div><h2 id="footprint-heading" className="flex items-center gap-2 font-display text-xl font-bold text-ink"><Footprints size={19} className="text-primary" /> 成长足迹</h2><p className="mt-1 text-xs text-faint">只记录目标、成就、阶段和主动添加的重要事件</p></div><Button variant="ghost" className="min-h-9 px-3" icon={<Plus size={14} />} onClick={() => setEventEditor('new')}>重要事件</Button></div>
        <Panel className="p-5 sm:p-6">
          {footprint.length > 0 ? (
            <ol className="relative space-y-0 before:absolute before:bottom-3 before:left-[15px] before:top-3 before:w-px before:bg-line">
              {footprint.map((event) => <li key={event.id} className="relative flex gap-4 pb-6 last:pb-0"><span className="z-10 flex size-8 shrink-0 items-center justify-center rounded-full border border-line bg-surface text-sm">{EVENT_ICONS[event.sourceType]}</span><div className="min-w-0 flex-1 pt-1"><div className="flex flex-wrap items-baseline justify-between gap-2"><h3 className="font-medium text-ink">{event.title}</h3><time className="text-xs text-faint">{formatDate(event.date)}</time></div>{event.description ? <p className="mt-1 text-sm leading-6 text-muted">{event.description}</p> : null}</div></li>)}
            </ol>
          ) : <p className="text-sm text-muted">还没有成长足迹。</p>}
        </Panel>
      </section>

      <details className="group rounded-xl border border-line bg-surface/60 p-4 sm:p-5">
