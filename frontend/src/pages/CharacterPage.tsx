    ...
    <div className="space-y-6 lg:space-y-7">
        ...
        <div className="flex items-center gap-4 sm:gap-5">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex flex-wrap items-baseline justify-between gap-2">
              ...
              <p className="text-sm text-muted">总 EXP <strong className="text-lg tabular-nums text-primary">{formatNumber(data.character.totalExp)}</strong></p>
            </div>
              ...
              return (
                <div key={category.id} className="p-4 sm:p-5">
                  ...
                  <ul className="mt-3 grid gap-2.5 sm:grid-cols-2">
                    {skills.map((skill) => (
                      <li key={skill.id} className="rounded-xl bg-raised p-3 ring-1 ring-white/6">
                        <div className="flex items-center justify-between gap-3">
                          <span className="truncate text-sm font-medium text-ink">{skill.name}</span>
                          <span className="shrink-0 rounded-full bg-arcane-soft px-2 py-0.5 text-xs font-semibold text-arcane">Lv.{skill.level}</span>
                        </div>
                        <div className="mt-2.5"><ProgressBar value={skill.exp} max={skill.expToNextLevel} size="sm" /></div>
                      </li>
                    ))}
                  </ul>
                </div>
      ...
      <section aria-labelledby="achievements-heading">
        <div className="mb-3 flex items-end justify-between gap-3">
          <div><h2 id="achievements-heading" className="flex items-center gap-2 text-lg font-semibold tracking-tight text-ink lg:text-xl"><Award size={19} className="text-muted" /> 成就徽章墙</h2><p className="mt-1 hidden text-xs text-faint lg:block">重要节点会自动解锁并写入成长足迹</p></div>
          <span className="text-xs text-muted">{unlockedAchievements.length}<span className="text-faint"> / {data.achievements.length} 已解锁</span></span>
        </div>
        {data.achievements.length > 0 ? (
          <div className="-mx-4 flex snap-x snap-mandatory gap-3 overflow-x-auto px-4 pb-1 sm:mx-0 sm:grid sm:snap-none sm:grid-cols-2 sm:overflow-visible sm:px-0 sm:pb-0 lg:grid-cols-3">
            {[...unlockedAchievements, ...data.achievements.filter((achievement) => achievement.unlockedAt === null)].map((achievement) => {
              const unlocked = achievement.unlockedAt !== null
              return (
                <Panel key={achievement.id} className={cn('flex w-[75vw] max-w-72 shrink-0 snap-start items-start gap-3 p-4 sm:w-auto sm:max-w-none', !unlocked && 'opacity-55')}>
                  <span className={unlocked
                    ? 'flex size-11 shrink-0 items-center justify-center rounded-full bg-[linear-gradient(150deg,rgb(245_184_61/0.22),rgb(139_124_246/0.22))] text-2xl ring-1 ring-primary/30'
                    : 'flex size-11 shrink-0 items-center justify-center rounded-full bg-white/4 text-2xl grayscale ring-1 ring-white/10'}>
                    {unlocked ? achievement.icon : '🔒'}
                  </span>
                  <div>
                    <h3 className={unlocked ? 'font-semibold text-ink' : 'font-semibold text-muted'}>{achievement.name}</h3>
                    <p className="mt-1 text-xs leading-5 text-muted">{achievement.description}</p>
                    <p className="mt-2 text-[11px] text-faint">{unlocked ? formatDate(achievement.unlockedAt) : '未解锁'}</p>
                  </div>
                </Panel>
              )
            })}
          </div>
        ) : <Panel className="p-6 text-sm text-muted">还没有成就规则。持续完成任务和长期目标即可自然获得。</Panel>}
      </section>
      ...
      <section aria-labelledby="footprint-heading">
        ...
        <Panel className="p-5 sm:p-6">
          {footprint.length > 0 ? (
            <ol className="relative space-y-0 before:absolute before:bottom-3 before:left-[15px] before:top-3 before:w-px before:bg-[linear-gradient(180deg,rgb(245_184_61/0.4),rgb(255_255_255/0.06))]">
              {footprint.map((event) => (
                <li key={event.id} className="relative flex gap-4 pb-6 last:pb-0">
                  <span className="z-10 flex size-8 shrink-0 items-center justify-center rounded-full bg-raised text-sm ring-1 ring-primary/25 shadow-[0_0_10px_rgb(245_184_61/0.15)]">{EVENT_ICONS[event.sourceType]}</span>
                  <div className="min-w-0 flex-1 pt-1">
                    <div className="flex flex-wrap items-baseline justify-between gap-2"><h3 className="font-medium text-ink">{event.title}</h3><time className="text-xs text-faint">{formatDate(event.date)}</time></div>
