      ...
      <section className="flex items-center gap-4 sm:gap-5">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-baseline justify-between gap-2">
            <h1 className="text-2xl font-semibold tracking-tight text-ink sm:text-3xl">Lv. {data.character.level}</h1>
            <span className="text-sm text-muted">{data.character.lifeStage}</span>
          </div>
          <div className="mt-2"><ProgressBar value={data.character.exp} max={data.character.expToNextLevel} label={`EXP ${formatNumber(data.character.exp)} / ${formatNumber(data.character.expToNextLevel)}`} /></div>
        </div>
      </section>
        ...
        <div className="space-y-6">
          ...
          <section aria-labelledby="today-tasks-heading">
            <div className="flex items-end justify-between gap-3 pb-3">
              <div>
                <h2 id="today-tasks-heading" className="text-xl font-semibold tracking-tight text-ink">今日待办</h2>
