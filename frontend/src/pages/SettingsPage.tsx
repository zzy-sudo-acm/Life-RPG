  ...
  return (
    <div className="space-y-7">
      <header className="border-b border-line pb-5">
        ...
        <h1 className="mt-3 font-display text-3xl font-bold text-ink">设置</h1>
        <p className="mt-2 text-sm text-muted">数据管理收纳在这里，不再占用主导航。</p>
      </header>

      <section><h2 className="font-display text-xl font-bold text-ink">存档概览</h2><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-5">{counts.map(([label, count]) => <div key={label} className="rounded-xl border border-line bg-surface px-3 py-3 text-center"><strong className="block text-xl text-ink">{count}</strong><span className="mt-1 text-xs text-muted">{label}</span></div>)}</div></section>

      {(feedback || storeError) ? <p role={feedback?.tone === 'error' || storeError ? 'alert' : 'status'} className={`rounded-xl border px-4 py-3 text-sm ${feedback?.tone === 'error' || storeError ? 'border-danger/30 bg-danger-soft text-danger' : 'border-primary/30 bg-primary-soft text-primary'}`}>{feedback?.message ?? storeError}</p> : null}

      <section className="grid gap-4 lg:grid-cols-2">
        <Panel className="p-5 sm:p-6"><div className="flex gap-3"><span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-primary-soft text-primary"><Download size={20} /></span><div><h2 className="font-semibold text-ink">导出备份</h2><p className="mt-1 text-sm text-muted">下载完整 JSON 存档，便于转移或长期保存。</p></div></div><Button className="mt-5" icon={<Download size={16} />} disabled={busy !== null} onClick={() => void handleExport()}>{busy === 'export' ? '正在导出…' : '导出存档'}</Button></Panel>
      ...
      <Panel className="border-danger/30 p-5 sm:p-6"><h2 className="font-semibold text-ink">恢复初始数据</h2><p className="mt-1 text-sm text-muted">当前存档会被覆盖，此操作无法在应用内撤销。</p><Button className="mt-5" variant="danger" icon={<RotateCcw size={16} />} disabled={busy !== null} onClick={() => void handleReset()}>{busy === 'reset' ? '正在重置…' : '重置全部数据'}</Button></Panel>
    </div>
  )
}
